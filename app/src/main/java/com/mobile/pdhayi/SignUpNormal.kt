    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.signUpBtn).setOnClickListener {
            if (firstName.text.isNullOrEmpty() || lastName.text.isNullOrEmpty() || email.text.isNullOrEmpty() || password.text.isNullOrEmpty() || passwordConfirm.text.isNullOrEmpty()) {
                ...
                UserRepository.registerUser(applicationContext, json,
                    onSuccess = { response ->
                        runOnUiThread {
                    ...
                    onFailure = { message, code ->
                        Log.e("SignUpNormal", "Registration failed: $message with $code")
                        runOnUiThread {
                            if (code == 400) {
        ...
        if (requestCode == RC_SIGN_IN) {
            ...
            try {
                ...
                if (account != null) {
                    ...
                    val firstName: String = if (nameParts.isNotEmpty()) nameParts[0] else "First"
                    val lastName: String = if (nameParts.size > 1) nameParts[1] else "Last"
                    ...
                    UserRepository.googleSignUp(this, json,
                        ...
                        onFailure = { code, message ->
                            Log.d("API ResponseE", "$code $message")
                            runOnUiThread {
                                if (code == 401) {
                                    Toast.makeText(applicationContext, "Email already exists, Please try to login", Toast.LENGTH_SHORT).show()
                                } else {
